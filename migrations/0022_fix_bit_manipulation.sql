-- Fix Bit Manipulation lesson - move video after intro text, keep table
UPDATE lessons 
SET content_json = '{
  "sections": [
    {
    ...
    },
    {
      ...
      "images": [{"url": "embedded:page13_img1", "alt": "BCD masking example", "caption": "PCF8563 Seconds Extraction Example"}]
    },
    {
      "title": "Other Bit Operations",
      "content": "In addition to masking, firmware often needs to modify individual bits without disturbing the rest of a register.\n\nSetting a bit is typically done using a bitwise OR operation, which forces a specific bit to one while leaving all others unchanged.\n\nClearing a bit is accomplished by ANDing the value with the inverse of a bit mask, forcing the selected bit to zero.\n\nToggling a bit uses the XOR operator to flip the bit current state.\n\nMore complex operations involve updating a bit field, where multiple adjacent bits represent a single value. In these cases, the existing bits are first cleared using a mask, and the new value is then shifted into position and ORed into the cleared space. This pattern is commonly used when writing configuration values to control registers.\n\nFinally, embedded systems often need to combine or split bytes and nibbles. Upper and lower nibbles can be extracted using masks and shifts, or combined by shifting one value into position and ORing it with another. These techniques are especially common when parsing multi-byte sensor data or constructing command packets for communication protocols.",
      "images": [
        {"url": "embedded:page14_img1", "alt": "Setting bit 3 example", "caption": "Setting Bits Example (Sets Bit 3 in Reg to 1)"},
        {"url": "embedded:page14_img2", "alt": "Clearing bit 2 example", "caption": "Clearing Bits Example (Forces Bit 2 in Reg to be 0)"},
        {"url": "embedded:page15_img1", "alt": "Toggling a bit example", "caption": "Toggling Bits Example (Flips Bit 1 in Reg to 0 or 1)"},
        {"url": "embedded:page15_img2", "alt": "Bit field manipulation", "caption": "Writing a Bit Field (Clear the Only Bit You Want to Update then OR the New Value into the Cleared Space)"},
        {"url": "embedded:page15_img3", "alt": "Combining nibbles", "caption": "Combining Two Nibbles (Upper/Lower Bytes)"}
      ]
    }
  ]
}'
